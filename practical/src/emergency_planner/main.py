...
class EmergencyPlannerFlow(Flow[EmergencyPlannerState]):
    def _create_initial_state(self) -> EmergencyPlannerState:
        return EmergencyPlannerState(emergency_call=None, fire_fighter_dispatch=None)

    @start()
    def receive_call(self):
        print("Receiving call")
        self.state.emergency_call = EmergencyCall(
            location="123 Main St", description="Fire", timestamp="2021-01-01 12:00:00"
        )
        result = (
            EmergencyServicesCrew()
            .crew()
            .kickoff(inputs={"emergency_call": self.state.emergency_call})
        )
    ...
    def plan_route(self):
        ...
        result = (
            FireFightersCrew()
            .crew()
            .kickoff(inputs={"fire_fighter_dispatch": self.state.fire_fighter_dispatch})
        )
