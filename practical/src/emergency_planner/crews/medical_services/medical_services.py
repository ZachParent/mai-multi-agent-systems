...
class MedicalServicesCrew:
    ...
    def fetch_hospital_information(self) -> Task:
        ...
        return Task(
            config=config,
            tools=[hospital_reader_tool],
            output_pydantic=HospitalsInformation,
        )
    ...
    def rank_hospitals(self) -> Task:
        ...
        return Task(
            config=config, tools=[route_distance_tool], output_pydantic=RankedHospitals
        )
    ...
    def plan_hospital_response(self) -> Task:
        ...
        return Task(config=config, output_pydantic=AllocatedHospitalResources)
    ...
    def deploy_paramedics(self) -> Task:
        ...
        return Task(config=config, output_pydantic=DeployedParamedics)
